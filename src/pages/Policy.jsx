...
function comparePolicyGroups(aGroup, bGroup) {
  ...
  if (normalizedA === OLD_POLICIES_GROUP && normalizedB !== OLD_POLICIES_GROUP) return 1;
  if (normalizedB === OLD_POLICIES_GROUP && normalizedA !== OLD_POLICIES_GROUP) return -1;
...
function Policy() {
  ...
  useEffect(() => {
    ...
    const loadPolicies = async () => {
      ...
      setPolicyData(sortPolicies(data && data.length ? data : DEFAULT_POLICY_DOCUMENTS));
      setIsLoading(false);
    };
  ...
  const groupNames = useMemo(
    () =>
      Object.keys(policyGroups).sort((a, b) => comparePolicyGroups(a, b)),
    [policyGroups]
  );
  ...
  const visibleGroupNames = groupNames.filter((group) => filteredGroups[group]?.length);
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-5">
          ...
          <div className="grid gap-6 p-6 lg:grid-cols-[1fr_360px]">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-2xl text-base leading-7 text-slate-600">
                  Browse the latest policy documents, filter by category, or search by title to find the right governance guide quickly.
                </p>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-sm font-semibold text-slate-500">Total documents</p>
                  <p className="mt-2 text-3xl font-bold text-slate-950">{totalPolicies}</p>
                </div>
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-sm font-semibold text-slate-500">Current view</p>
                  <p className="mt-2 text-3xl font-bold text-slate-950">{filteredCount}</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-6 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Use search and group filters to narrow down the cooperative policies instantly.
              </p>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-700">Policy results</p>
              <p className="mt-1 text-sm text-slate-500">
                Showing {filteredCount} {filteredCount === 1 ? 'document' : 'documents'} across {visibleGroupNames.length} {visibleGroupNames.length === 1 ? 'group' : 'groups'}.
              </p>
            </div>
