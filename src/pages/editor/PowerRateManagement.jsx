        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
          ...
          <div className="border-b border-slate-200 bg-white px-5 py-5 sm:px-7">
            ...
            <div className="flex flex-wrap gap-2">
              {years.map((year) => {
                const active = selectedYear?.id === year.id;
                return (
                  <button key={year.id} type="button" onClick={() => handleSelectYear(year)} className={`rounded-xl px-5 py-2.5 text-sm font-bold transition ${active ? "bg-slate-900 text-white shadow-sm" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                    {year.year}
                  </button>
                );
              })}
            ...
            <>
              ...
              <div className="border-b border-slate-200 bg-slate-50 px-5 py-5 sm:px-7">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedYear.pdf_url && (
                      <a href={selectedYear.pdf_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:bg-slate-100">
              ...
              <div className="border-b border-slate-200 bg-white px-5 sm:px-7">
                <div className="flex overflow-x-auto">
                  {RATE_CLASSES.map((rateClass) => {
                    const active = activeRateClass === rateClass.id;
                    return (
                ...
                <table className="w-full min-w-[650px] border-collapse">
                  <thead>
                    <tr className="bg-slate-900 text-white">
                      ...
                      <th className="px-5 py-4 text-left text-xs font-bold uppercase tracking-wider">
                        {RATE_CLASSES.find((item) => item.id === activeRateClass)?.label}{" "}
                        Rate (PHP/kWh)
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {MONTHS.map((month, index) => {
                      const value = rates?.[activeRateClass]?.[month.number] ?? "";
                      return (
                        <tr key={month.number} className={`border-b border-slate-200 ${index % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                          {/* Month */}
                          <td className="px-5 py-4">
                            ...
                            <div className="text-xs font-semibold text-slate-400">
                              {month.short} {selectedYear.year}
                            </div>
                          </td>

                          {/* Rate */}
                          <td className="px-5 py-4">
                            <input type="number" step="0.0001" min="0" value={value} placeholder="N/A" onChange={(e) => handleRateChange(month.number, e.target.value)} className="w-full max-w-xs rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-100" />
                          </td>
                        </tr>
                      );
                    })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setShowAddYear(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                <FaTimes />
              </button>
            </div>
            ...
            <form onSubmit={handleAddYear}>
              <div className="space-y-5">
                ...
                <label className="block">
                  ...
                  <input type="number" min="2000" max="2100" value={yearForm.year} onChange={(e) => setYearForm({
                    ...yearForm,
                    year: e.target.value,
                  })} placeholder="2027" className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100" required />
                </label>
                ...
                <label className="block">
                  ...
                  <input type="url" value={yearForm.pdf_url} onChange={(e) => setYearForm({
                    ...yearForm,
                    pdf_url: e.target.value,
                  })} placeholder="https://drive.google.com/..." className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100" />
                </label>
              </div>

              <div className="mt-7 flex justify-end gap-3">
                <button type="button" onClick={() => setShowAddYear(false)} className="rounded-xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-600 hover:bg-slate-100">
                  Cancel
                </button>
                <button type="submit" disabled={saving} className="rounded-xl bg-amber-500 px-5 py-3 text-sm font-bold text-white hover:bg-amber-600 disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setShowEditYear(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                <FaTimes />
              </button>
            </div>
              ...
              <div className="space-y-5">
                ...
                <label className="block">
                  ...
                  <input type="number" min="2000" max="2100" value={yearForm.year} onChange={(e) => setYearForm({
                    ...yearForm,
                    year: e.target.value,
                  })} className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100" required />
                </label>
