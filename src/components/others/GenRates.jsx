...
const GenRates = () => {
  ...
  const loadGenerationCharges = async () => {
    try {
      ...
      setError("Unable to load the generation charge breakdown.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            ...
            <button
              ...
              disabled={generationCharges.length === 0}
            ...
            <div className="flex h-10 min-w-[100px] items-center justify-center rounded-lg border border-slate-600 bg-slate-950/60 px-4">
              <span className="text-sm font-semibold text-white">
                {generationCharges.length > 0 ? activePage + 1 : 0}
              </span>
      ...
      <div className="p-4 sm:p-6">
        ...
        {/* EMPTY */}
        {!loading && !error && generationCharges.length === 0 && (
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                Page {activeGenerationCharge?.display_order}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-200 bg-white px-4 py-3">
              ...
              {/* DOT INDICATORS */}
              <div className="flex items-center gap-1.5" aria-label="Generation charge pages">
                {generationCharges.map((generationCharge, index) => (
                  <button
                    key={generationCharge.id}
                    type="button"
                    onClick={() => setActivePage(index)}
                    aria-label={`View generation charge page ${generationCharge.display_order}`}
                    aria-current={index === activePage ? "true" : undefined}
                    className={`h-2 rounded-full transition-all duration-200 ${index === activePage ? "w-6 bg-amber-500" : "w-2 bg-slate-300 hover:bg-slate-400"}`}
                  />
                ))}
              </div>
