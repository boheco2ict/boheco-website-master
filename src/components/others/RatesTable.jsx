        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold uppercase tracking-[0.16em] text-amber-600">Monthly Energy Rate</span>
            </div>
            ...
            <p className="mt-1 text-sm text-slate-500">Monthly rate summary in PHP per kWh.</p>
          </div>
          {url && (
            <a href={url} target="_blank" rel="noreferrer" className="inline-flex w-fit items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-amber-300 hover:bg-amber-50 hover:text-amber-700 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2">
          ...
          <thead>
            <tr className="bg-slate-950">
              {thead.map((item, index) => {
                const isAverage = index === thead.length - 1;
                return (
                  <th key={index} className={`whitespace-nowrap px-4 py-3.5 text-center text-[10px] font-extrabold uppercase tracking-wider ${isAverage ? "bg-amber-500 text-white" : "text-slate-300"}`}>
                    {item}
                  </th>
                );
              })}
          ...
          <tbody>
            <tr className="border-b border-slate-100 bg-white transition-colors duration-200 hover:bg-amber-50/30">
              {tbody.map((item, index) => {
                const isAverage = index === tbody.length - 1;
                const isEmpty = item === null || item === undefined || item === "";
                return (
                  <td key={index} className={`whitespace-nowrap px-4 py-4 text-center ${isAverage ? "bg-amber-50/60 font-extrabold text-amber-700" : "font-semibold text-slate-700"}`}>
                    {isEmpty ? <span className="text-xs font-medium text-slate-400">N/A</span> : item}
                  </td>
                );
              })}
            </tr>
